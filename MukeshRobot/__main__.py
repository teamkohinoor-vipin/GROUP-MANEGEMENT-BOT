...
def start(update: Update, context: CallbackContext):
    ...
    if update.effective_chat.type == "private":
        if len(args) >= 1:
            ...
            elif args[0].lower().startswith("stngs"):
...
def Mukesh_about_callback(update: Update, context: CallbackContext):
    ...
    if query.data == "mukesh_":
        uptime = get_readable_time((time.time() - StartTime))
        # Fixed: removed commas that were splitting the string into multiple arguments
        query.message.edit_caption(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    ...
                    [
                        InlineKeyboardButton(
                            text="ᴜᴩᴅᴀᴛᴇs 🍷",
                            url="\x68\x74\x74\x70\x73\x3A\x2F\x2F\x74\x2E\x6D\x65\x2F\x6D\x75\x6B\x65\x73\x68\x62\x6F\x74\x7A\x6F\x6E\x65",
                        ),
                    ],
                    [
                        InlineKeyboardButton(
...
© 2023 - 2024 | sᴜᴘᴘᴏʀᴛ ᴄʜᴀᴛ, ᴀʟʟ ʀɪɢʜᴛs ʀᴇsᴇʀᴠᴇᴅ.
""",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            text="sᴜᴘᴘᴏʀᴛ ᴄʜᴀᴛ",
                            url="https://t.me/+-vprAIeEo1c2NTBl",
                        )
                    ],
                    [
                        InlineKeyboardButton(text="🏡", callback_data="mukesh_back"),
                    ...
                    [
                        ...
                        InlineKeyboardButton(
                            text="𝚂𝚞𝚙𝚙𝚘𝚛𝚝 𝙲𝚑𝚊𝚗𝚗𝚎l",
                            url="https://t.me/osnitInfo",
                        ),
                    ],
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    ...
                    [
                        ...
                        InlineKeyboardButton(
                            text="⍟ ᴇxᴛʀᴀ ⍟", callback_data="Music_extra"
                        ),
                    ],
                    [
                        InlineKeyboardButton(text="• ʙᴀᴄᴋ •", callback_data="Main_help"),
...
def get_help(update: Update, context: CallbackContext):
    ...
    # ONLY send help in PM if chat.type != chat.PRIVATE:
...
def get_settings(update: Update, context: CallbackContext):
    ...
    # ONLY send settings in PM if chat.type != chat.PRIVATE:
    if chat.type != chat.PRIVATE:
...
def main():
    ...
    # Removed startup message to support chat
